...
class UserTypeCheck(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if not employer_id or not validationClass.is_valid_uuid(
            ...
            self.logger.error("Invalid employer_id or UUID validation failed")
            return False
        ...
        if (
            ...
            self.logger.error("Permission denied! User is not an employer")
            return False

        self.logger.info(f"Permission granted for the user")
        return super().has_permission(request, view)

    @staticmethod
    def is_user_employer(self, user_id):
        """Check if the user_id belongs to employer"""

        # check if the user_id belongs to any user
        self.logger.info(f"Checking if user_id {user_id} belongs to an employer")
        user_data = User.objects.filter(user_id=user_id, user_type__iexact="employer")
        if user_data.exists():
            self.logger.info(f"user_id {user_id} belongs to an employer")
            return True
        else:
            self.logger.error(f"user_id {user_id} does not belong to an employer")
            return False
